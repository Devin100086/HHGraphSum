...
class SGATLayer(nn.Module):
    ...
    def edge_attention(self, edges):
        dfeat = self.feat_fc(edges.data["simidfembed"])  # [edge_num, out_dim]
...
class WGATLayer(nn.Module):
    def __init__(self, in_dim, out_dim, weight=0):
        super(WGATLayer, self).__init__()
        self.weight = weight
        self.fc = nn.Linear(in_dim, out_dim, bias=False)
        self.attn_fc = nn.Linear(2 * out_dim, 1, bias=False)

    def edge_attention(self, edges):
        # dfeat = self.feat_fc(edges.data["nextidembed"])           # [edge_num, out_dim]
    ...
    def forward(self, g, h):
        ...
        wedge_id = g.filter_edges(lambda edges: edges.data["dtype"] == 2)
        z = self.fc(h)
        g.nodes[wnode_id].data['z'] = z
        g.apply_edges(self.edge_attention, edges=wedge_id)
...
class WSGATLayer(nn.Module):
    def __init__(self, in_dim, out_dim, feat_embed_size):
        ...
        self.fc2 = nn.Linear(64, out_dim, bias=False)
    ...
    def forward(self, g, dsth, srch):
        ...
        # z = self.fc(h)
        g.nodes[wnode_id].data['z'] = self.fc1(srch)
        g.nodes[snode_id].data['z'] = self.fc2(dsth)
...
class SWGATLayer(nn.Module):
    def __init__(self, in_dim, out_dim, feat_embed_size):
        super().__init__()
        self.fc1 = nn.Linear(in_dim, out_dim, bias=False)
        self.fc2 = nn.Linear(300, out_dim, bias=False)
    ...
    def forward(self, g, dsth, srch):
        ...
        # z = self.fc(h)
        g.nodes[snode_id].data['z'] = self.fc1(srch)
        g.nodes[wnode_id].data['z'] = self.fc2(dsth)
