...
def get_encoder_mask(src_len):
    valid_encoder_mhsa_pos = torch.vstack([
        F.pad(torch.ones(L), (0, max(src_len) - L)) for L in src_len
    ]).unsqueeze(-1) 

    encoder_mhsa_mask = torch.bmm(valid_encoder_mhsa_pos, valid_encoder_mhsa_pos.transpose(-2, -1))
    encoder_mhsa_mask = encoder_mhsa_mask.unsqueeze(1)
    return encoder_mhsa_mask
# ----------------------------------
