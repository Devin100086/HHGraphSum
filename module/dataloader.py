...
class ExampleSet(torch.utils.data.Dataset):
    ...
    def AddWordNode(self, G, inputid):
        ...
        return wid2nid, nid2wid, sent_Len
    
    def AddWordEdge(self, G, nid2wid, sent_Len):
        # add next edge
        index = 0
        for i,nid in enumerate(nid2wid.keys()):
            if i + 1 >= sent_Len[index]:
                index += 1
                continue
            else:
                G.add_edges(i, i+1, data={"dtype": torch.Tensor([2])})
    ...
    def CreateGraph(self, input_pad, label, w2s_w):
        ...
        """
        G = dgl.DGLGraph()
        wid2nid, nid2wid, sent_Len = self.AddWordNode(G, input_pad)
        self.AddWordEdge(G, nid2wid, sent_Len)
...
class MultiExampleSet(ExampleSet):
    ...
    def __init__(self, data_path, vocab, doc_max_timesteps, sent_max_len, filter_word_path, w2s_path, w2d_path, s2s_path):
        """ Initializes the ExampleSet with the path of data
        ...
        logger.info("[INFO] Loading sent2sent Similar file from %s!" % s2s_path)
        self.s2s_sim = readJson(s2s_path)
    ...
    def CreateGraph(self, docLen, sent_pad, doc_pad, label, w2s_w, w2d_w, s2s_s):
        """ Create a graph for each document
        ...
        # add word nodes
        G = dgl.DGLGraph()
        wid2nid, nid2wid, sent_Len = self.AddWordNode(G, sent_pad)
        self.AddWordEdge(G, nid2wid, sent_Len)
        w_nodes = len(nid2wid)
        ...
        for i in range(N):
            ...
            G.add_edges(sent_nid, sentid2nid, data={"dtype": torch.ones(N), "simfrac": 
                                                    torch.round(torch.abs(s2s_s[sent_nid-w_nodes, :])*9).to(torch.int64)})
            G.add_edges(sentid2nid, sent_nid, data={"dtype": torch.ones(N), "simfrac":
                                                    torch.round(torch.abs(s2s_s[:, sent_nid-w_nodes])*9).to(torch.int64)})        
            # s2d
            docid = sent2doc[i]
            docnid = docid2nid[docid]
            G.add_edges(sent_nid, docnid, data={"tffrac": torch.LongTensor([1]), "dtype": torch.Tensor([2])})
    ...
    def __getitem__(self, index):
        ...
        label = self.pad_label_m(item.label_matrix)
        w2s_w = self.w2s_tfidf[index]
        w2d_d = self.w2d_tfidf[index]
        s2s_s = torch.FloatTensor(self.s2s_sim[index])[:self.doc_max_timesteps, :self.doc_max_timesteps]
        G = self.CreateGraph(article_len, sent_pad, enc_doc_input, label, w2s_w, w2d_d, s2s_s)
        return G, index
...
# Function to generate trigrams from a sentence  
