...
def run_training(model, train_loader, valid_loader, valset, hps, train_dir):
    ...
    optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=hps.lr, betas=(0.9, 0.999))
...
def run_eval(model, loader, valset, hps, best_loss, best_F, non_descent_cnt, saveNo):
    ...
    with torch.no_grad():
        ...
        for i, (G, index) in enumerate(tqdm(loader)):
            if hps.cuda:
                G = G.to(torch.device("cuda"))
    ...
    scores_all = utils.rouge_corpus(tester.refer, tester.hyps)
...
def main():
    ...
    # Where to find data
    parser.add_argument('--data_dir', type=str, default='data/MultiNews',help='The dataset directory.')
    parser.add_argument('--cache_dir', type=str, default='cache/MultiNews',help='The processed dataset directory')
    ...
    # Important settings
    parser.add_argument('--model', type=str, default='HDSG', help='model structure[HSG|HDSG]')
    ...
    parser.add_argument('--sent_max_len', type=int, default=500,help='max length of sentences (max source text sentence tokens)')
    parser.add_argument('--doc_max_timesteps', type=int, default=50,help='max length of documents (max timesteps of documents)')
    parser.add_argument('--transformer_max_len', type=int, default=150,help='max length of transformer encoder [default: 150]')
    parser.add_argument('--transformer_head', type=int, default=8, help='number of epochs [default: 100]')
    parser.add_argument('--transformer_dropout', type=float, default=0.1,help='dropout of transformer [default: 0.1]')
    parser.add_argument('--transformer_num_layers', type=int, default=3,help='layer number of transformer [default: 100]')
    ...
    parser.add_argument('-m', type=int, default=9, help='decode summary length')
    ...
    train_s2s_path = os.path.join(args.cache_dir, "train.s2s.sim.jsonl")
    val_s2s_path = os.path.join(args.cache_dir, "val.s2s.sim.jsonl") 

    if hps.model == "HHGS":
        model = HHGraphSum(hps, embed)
        logger.info("[MODEL] HeterSumGraph ")
        dataset = ExampleSet(DATA_FILE, vocab, hps.doc_max_timesteps, hps.sent_max_len, FILTER_WORD, train_w2s_path, train_s2s_path)
        ...
        del dataset
        valid_dataset = ExampleSet(VALID_FILE, vocab, hps.doc_max_timesteps, hps.sent_max_len, FILTER_WORD, val_w2s_path, val_s2s_path)
    ...
    elif hps.model == "HHDGS":
        model = HHDocGraphSum(hps, embed)
        logger.info("[MODEL] HeterDocSumGraph ")
        train_w2d_path = os.path.join(args.cache_dir, "train.w2d.tfidf.jsonl")
        dataset = MultiExampleSet(DATA_FILE, vocab, hps.doc_max_timesteps, hps.sent_max_len, FILTER_WORD, train_w2s_path, train_w2d_path, train_s2s_path)
        train_loader = torch.utils.data.DataLoader(dataset, batch_size=hps.batch_size, shuffle=True, num_workers=32,collate_fn=graph_collate_fn)
        del dataset
        val_w2d_path = os.path.join(args.cache_dir, "val.w2d.tfidf.jsonl")
        valid_dataset = MultiExampleSet(VALID_FILE, vocab, hps.doc_max_timesteps, hps.sent_max_len, FILTER_WORD, val_w2s_path, val_w2d_path, val_s2s_path)
